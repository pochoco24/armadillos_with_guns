...
var substate = Substate.FALLING: set = set_substate
...
func enter():
	...
	# Ball always starts in mid air when player goes into ball mode
	ball.on_floor = false
...
func set_substate(new_substate: Substate):
	if new_substate == substate:
		return
	print(Substate.keys()[new_substate])
...
func physics_update(delta):
	...
	if Input.is_action_just_pressed("jump"):
		print("JUMPEDDDDDDDDDDDDDDDDDDDDDDD", Substate.keys()[substate], ball.on_floor)
	...
	if substate == Substate.ROLLING:
		...
		if ball.on_floor:
			if not player.jump_buffer.is_stopped() or Input.is_action_just_pressed("jump"):
				set_substate(Substate.JUMPING)
			else:
